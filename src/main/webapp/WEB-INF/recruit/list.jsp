...
<script>
	var pageHelper;
	function page() {
		var url = "recruit/selectAll";
		//后面两个参数非必需
		//会自动调用一次loadData()
		pageHelper = pageHelpers(
				url,
				function(data) {
					//这里放渲染html的方法
					var table = $("#xxxxxxx");
					...
					for (var i = 0; i < data.length; i++) {
		                ...
		                + "<td>" + data[i].title + "</td>"
		                + "<td>" + data[i].salarymin + "</td>"
		                + "<td>" + data[i].salarymax + "</td>"
		                + "<td>" + data[i].jobdes + "</td>"
		                + "<td>" + data[i].qualifications + "</td>"
		                + "<td>" + data[i].rtime + "</td>"
		                + "<td>"
								...
								+ "			<span class='am-icon-pencil-square-o'></span> <a href='recruit/selecRecruitinformationtAll?rid="
								+ data[i].rid
								+ "'>详情</a>"
								+ "		</button>"
								+ "		<button"+
								"class='am-btn am-btn-default am-btn-xs am-hide-sm-only>"
								+ "			<span class='am-icon-copy'></span><a href='recruit/showRid?rid="
								+ data[i].rid
								+ "'>修改"
								+ '</a>'
								+ "		</button>"
								+ "		<button"+
								"class='am-btn am-btn-default am-btn-xs am-hide-sm-only>"
								+ "			<span class='am-icon-copy'></span><a href='recruit/delete?rid="
								+ data[i].rid
								+ "'>删除"
								+ '</a>'
								+ "		</button>"
							...
							<div class="am-btn-toolbar">
								<div class="am-btn-group am-btn-group-xs">
									<button type="button" class="am-btn am-btn-default">
										<a href="recruit/add"> <span class="am-icon-plus"></span> 新增
										</a>
									</button>
								</div>
							</div>
					...
					<div class="am-g">
						<div class="am-u-sm-12">
							<table id="xxxxx"
								...
								<thead>
									<tr>
										<th class="table-title">公司id</th>
										<th class="table-type">工作</th>
										<th class="table-set">地址</th>
										<th class="table-title">福利</th>
										<th class="table-type">标题</th>
										<th class="table-title">最低工资</th>
										<th class="table-title">最高工资</th>
										<th class="table-type">详细介绍</th>
										<th class="table-set">资格</th>
